...
struct SummedPauliOp
{
  ...
  void __check_ctor_inputs(std::vector<fast_pauli::PauliString> const& pauli_strings, Tensor<2> const coeffs)
  {
    ...
    bool const qubits_match =
      std::all_of(pauli_strings.begin(), pauli_strings.end(), [n_qubits](fast_pauli::PauliString const& ps) {
   ...
   * @param coeffs
   */
  SummedPauliOp(std::vector<PauliString> const& pauli_strings, Tensor<2> const coeffs)
    : pauli_strings(pauli_strings)
  {
    //
    this->__check_ctor_inputs(pauli_strings, coeffs);
  ...
  void apply_parallel(Tensor<2> new_states,
                      Tensor<2> states,
...
#pragma omp parallel for collapse(2)
  ...
  // TODO IMPLEMENT
  template<std::floating_point data_dtype>
  void apply_parallel_weighted_data(Tensor<2> new_states,
                                    Tensor<2> states,
