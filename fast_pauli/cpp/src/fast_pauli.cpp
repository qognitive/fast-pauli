...
NB_MODULE(_fast_pauli, m)
{
  ...
  nb::class_<fp::PauliString>(m, "PauliString")
    ...
    .def(
      "__matmul__",
      [](fp::PauliString const& self, fp::PauliString const& rhs) { return self * rhs; },
      nb::is_operator())
    .def(
      "__matmul__",
      [](fp::PauliString const& self, fp::PauliOp<float_type> const& rhs) { return self * rhs; },
      nb::is_operator())
    .def(
      "__add__",
    ...
    .def(
      ...
      [](fp::PauliString const& self, nb::ndarray<cfloat_t> states, cfloat_t c) {
        ...
        if (states.ndim() == 1) {
          ...
          throw std::invalid_argument(fmt::format("apply: expected 1 or 2 dimensions, got {}", states.ndim()));
        }
      },
      "states"_a,
      "coeff"_a = cfloat_t{ 1.0 })
    .def(
  ...
  nb::class_<fp::PauliOp<float_type>>(m, "PauliOp")
    ...
    .def(
      "__init__",
      [](fp::PauliOp<float_type>* new_obj, std::vector<cfloat_t> coeffs_vec, std::vector<std::string> const& strings) {
        std::vector<fp::PauliString> pauli_strings;
        std::transform(strings.begin(), strings.end(), std::back_inserter(pauli_strings), [](std::string const& pauli) {
    ...
    .def(
      "__matmul__",
      [](fp::PauliOp<float_type> const& self, fp::PauliString const& rhs) { return self * rhs; },
      nb::is_operator())
    .def(
      "__add__",
  ...
  helpers_m.def(
    "calculate_pauli_strings_max_weight", &fp::calculate_pauli_strings_max_weight, "n_qubits"_a, "weight"_a);
